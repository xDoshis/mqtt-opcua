...
function Matcher(handlers) {
    ...
    this.matcher = new Qlobber({
        separator: '/',     //the character to use in separating words in topics, MQTT uses '/'
        wildcard_one: '+',  //character to use for matching exactly one non-empty word in a topic, MQTT uses '+'
        wildcard_some: '#'  //character to use for matching zero or more words in a topci 
    });
    ...
    function post_initialize() {
        ...
        function onMessage(topic, payload) {
            ...
            var sub,
                ...
                if(paths.hasOwnProperty(top) === true){
                    node = paths[top];
                }
                else{
                    node = nameSpace.addFolder("ObjectsFolder", {browseName: top});
                }
            ...
            for (sub in path) {
                ...
                if (topic !== top) {
                    if(paths.hasOwnProperty(top) === true){
                        ...
                        node = nameSpace.addFolder(node, {browseName: path[sub]});
                    }
                    ...
                    flagged[topic] = false;
                    try {
                        nodes[topic] = server.engine.nameSpace.addVariable({
                            componentOf: node,
                            browseName: path[sub],
                            path: topic,
                            nodeId: "s=" + topic,
